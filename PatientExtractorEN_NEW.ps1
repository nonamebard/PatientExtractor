...
function Find-FolderInArchives {
    ...
    for ($i = 0; $i -lt $allArchives.Count; $i++) {
        ...
        # FIXED: Search for folder patterns - remove problematic backslash pattern
        # 7z output typically uses forward slashes for paths
        $searchPatterns = @("$folderName/", " $folderName ")
        
        foreach ($pattern in $searchPatterns) {
            # Escape special regex characters in the folder name
            $escapedPattern = [regex]::Escape($pattern)
            if ($archiveContent -match $escapedPattern) {
                $foundArchives += @{
                    Path = $archive
                    Name = $archiveName
                    Folder = $folderName
...
function Show-MainForm {
    ...
    $folderLabel.Text = "Enter folder name (ID):"
    ...
    $exitButton.Location = New-Object System.Drawing.Point(20, 390)
    ...
    $searchButton.Add_Click({
        ...
        $statusLabel.Text = "Searching for folder '$folderName'..."
        $statusLabel.ForeColor = [System.Drawing.Color]::Blue
        $resultTextBox.Text = "Searching for folder '$folderName'...`r`n`r`n"
        ...
        if ($foundArchives.Count -eq 0) {
            ...
            $resultTextBox.AppendText("Please check the folder name and try again.`r`n")
            $statusLabel.Text = "Folder not found"
            $statusLabel.ForeColor = [System.Drawing.Color]::Red
            
            [System.Windows.Forms.MessageBox]::Show(
                "Folder '$folderName' not found in any archive!",
                "Search Result",
                "OK",
                "Information"
            )
            ...
            $resultTextBox.AppendText("Found in archive(s):`r`n")
            ...
            if ($foundArchives.Count -eq 1) {
                ...
                $confirm = [System.Windows.Forms.MessageBox]::Show(
                    "Folder found in archive: $($archiveToExtract.Name)`r`n`r`nExtract and copy to VARIANCOM server?`r`nDestination: $destinationServer",
                    "Confirmation",
                    "YesNo",
                    "Question"
                )
